...
#### Демонстрация работы
У клиента есть параметры 

```
  input.txt          файл
  --port 12000     порт сервера
  --chunk-size 20  размер одного пакета данных
...
#### Решение
Увеличение от $\frac{W}{2}$ к $W$ обычно идет по $1$ за один RTT, так что
$T = \frac{W}{2}$, если считать $T$ в RTT. Среднее окно за цикл: $B = \frac{\frac{W}{2} + W}{2} = \frac{3W}{4}$. Значит, $W$ можно выразить через $B$, а значит и $T$ тоже можно выразить через $B$.
...
#### Решение
Заметим что: $15S = S + 2S + 4S + 8S$, базовое время это $2RTT + \frac{15S}{R}$ (первое слагаемое - установка соединения и запрос, второе - передача данных)

Разберем случаи:

1. $4S/R > S/R + RTT > 2S/R$ значит простой есть после окон 1S и 2S, но нет после 4S

2. $S/R + RTT > 4S/R$ значит простой есть после окон 1S, 2S и 4S.

3. $S/R > RTT$ простоя нет.


Тогда в первом случае ответ: 

$T = 2RTT + 15S/R + RTT + (RTT - S/R) = 4RTT + 14S/R$

Во втором:

$T = 2RTT + 15S/R + RTT + (RTT - S/R) + (RTT - 3S/R) = 5RTT + 11S/R$
...
Во первых число шагов такого алгоритма не зависит очевидно от $W$, так как $k = log_{1 + a} 2$

А отправленное число пакетов это что то типа $N = W/2 + (W/2)(1+a) + ... + W$ = $W(2a + 1) / (2a)$ (сумма геом прогрессии + чуть пошаманили(а именно так как $(1 + a)^k = 2)$),

Если одна потеря происходит за цикл, то частота потерь: $L = 1/N$, значит 

$L = 2a / (W(2a + 1))$, что то такое.
