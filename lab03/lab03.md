...
Проверка ошибки 404:
```text
http://127.0.0.1:8080/no-such-file.html
```
(Только если будете запускать лучше указывать другие порты разные для запусков))

![Браузер](images/correct_file.png)
![Браузер](images/incorrect_file.png)


![Консоль](images/screen1.png)
![Консоль](images/screen2.png)
...
Код многопоточного сервера находится в файле `server.py`.
...
Теперь сервер не завершается после первого запроса, а создает отдельный поток для каждого
нового подключения. Тестировать проще в консоли, в примере просто 3 запроса.

![Консоль](images/task2.png)
![Консоль](images/task22.png)
...
```bash
python3 client.py 127.0.0.1 8080 no-such-file.html
```

![Консоль](images/task33.png)
...
Запуск сервера с числом потоков N = 2:
```bash
python3 server.py 8080 2
```

После этого можно отправить несколько запросов клиентом:
```bash
python3 client.py 127.0.0.1 8080 index.html
python3 client.py 127.0.0.1 8080 no-such-file.html
```

![Консоль](images/task44.png)
Так как это просто так не очень понятно как тестировать, в server/handle_client() в конце заккоменчена строка sleep, на фото я отправил 3 запроса, 2 выполнилось и мы ждем. Это как раз ожидаемое поведение.
...
#### Решение
Время чтобы собрать один пакет: $\frac{(56 \times 8)}{128000} \approx 3.5$ мс, передача аналогично $\frac{448}{1000000} \approx 0.45$ мс, ну и добавить задержку распространения($5$ мс). Итого порядка $8.95$ мс.
...
#### Решение
Время передачи одного пакета $\approx \frac{1}{100} = 0.01$ с $ = 10$ мс. 
Средняя общая задержка пакета = $10$ мс + $10$ мс (задержка ожидание + задержка передачи)
Тогда по формуле $a = \frac{N}{d} = (10 + 1) / 0.02 \approx 550$ пакетов/с.  (10 + 1 потому что еще текущий пакет)
