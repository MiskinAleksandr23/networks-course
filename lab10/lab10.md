...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - IP-заголовок: `20 bytes`.
   - `Total Length = 56 bytes` => полезная нагрузка IP-дейтаграммы: `56 - 20 = 36 bytes`.
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Меняются `Identification` и `Header Checksum`. При переходе к следующему hop также меняется `TTL`.
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются: `Version`, `Header Length`, `Total Length`, `Source Address`, `Destination Address`, `Protocol`, `Flags`, `Fragment Offset`.
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - `Identification` увеличивается: `0x9570 (38256)` -> `0x9571 (38257)`.
      - Закономерность: для последовательных исходящих дейтаграмм значение увеличивается на `1`.
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - Для пакета `239`: `Identification = 0x9570 (38256)`.
   - `TTL = 1`.
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Для ответов от первого маршрутизатора `172.20.10.1` значение `TTL` остается одинаковым (`64`), а `Identification` может меняться от одного ICMP-сообщения к другому.
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - Для ICMP-пакета `240`: `Identification = 0x946e (37998)`.
   - `TTL = 64`.
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да было фрагментировано. Было создано `3` IP-фрагмента: `#17`, `#18`, `#19`.
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - В разных фрагментах изменяются `Total Length`, `Flags`, `Fragment Offset` и `Header Checksum`. Поле `Identification` у фрагментов одного исходного пакета остается одинаковым: `0x97fe (38910)`.
...
#### Демонстрация работы
Запуск для первого хоста:

```
  sudo python3 icmp_ping.py 1.1.1.1 -c 4
```

  Вывод:
```
  PING 1.1.1.1 (1.1.1.1)
  32 bytes from 1.1.1.1: icmp_seq=1 time=58.325 ms
  32 bytes from 1.1.1.1: icmp_seq=2 time=55.282 ms
  32 bytes from 1.1.1.1: icmp_seq=3 time=40.050 ms
  32 bytes from 1.1.1.1: icmp_seq=4 time=40.820 ms
```
  Запуск для второго хоста:
```
  sudo python3 icmp_ping.py 8.8.8.8 -c 4
```
  Вывод:
```
  PING 8.8.8.8 (8.8.8.8)
  32 bytes from 8.8.8.8: icmp_seq=1 time=65.855 ms
  32 bytes from 8.8.8.8: icmp_seq=2 time=54.591 ms
  32 bytes from 8.8.8.8: icmp_seq=3 time=52.331 ms
  32 bytes from 8.8.8.8: icmp_seq=4 time=56.505 ms
```
...
#### Демонстрация работы
```
(base) near@MacBook-Pro-Alex lab10 % sudo python3 icmp_ping.py 8.8.8.8 -c 4
PINGING 8.8.8.8 (8.8.8.8)
32 bytes from 8.8.8.8: icmp_seq=1 time=69.007 ms
32 bytes from 8.8.8.8: icmp_seq=2 time=89.056 ms
32 bytes from 8.8.8.8: icmp_seq=3 time=56.891 ms
32 bytes from 8.8.8.8: icmp_seq=4 time=68.725 ms

--- 8.8.8.8 ping statistics ---
4 packets transmitted, 4 received, 0.0% packet loss
rtt min/avg/max = 56.891/70.920/89.056 ms
```
